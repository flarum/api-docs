...
export FLARUM_CORE_PATH="${FLARUM_PATH}/framework/core";

echo -e "${style}Using flarum/framework @ $FLARUM_CORE_PATH$reset"

if [[ -z "${GITHUB_ENV}" ]]; then
    echo -e "${style}Assuming local environment. \$GITHUB_ENV not set.$reset"
else
    echo "FLARUM_SHA=$(cd $FLARUM_CORE_PATH && git rev-parse --verify HEAD --short)" >> $GITHUB_ENV
fi
